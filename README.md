# Aspiring .NET with OpenAI and Ollama

A talk on building AI-powered, cloud-native applications with .NET Aspire, OpenAI, and Ollama, including an MCP server and Microsoft Agent Framework integration.

## Slides

[View HTML slides](https://chris-ayers.com/build-with-aspire/) | [Download PDF](https://chris-ayers.com/build-with-aspire/Slides.pdf)

## Resources

- [Slide source](slides/Slides.md)
- [Demo and setup guide](docs/demo-guide.md)
- [API test examples](api-tests/)
- [MCP server documentation](src/BuildWithAspire.MCPServer/README.md)
- [.NET Aspire](https://learn.microsoft.com/en-us/dotnet/aspire/get-started/aspire-overview)
- [Aspire Samples](https://github.com/dotnet/aspire-samples)
- [MCP Specification](https://modelcontextprotocol.io/)
- [MCP C# SDK](https://github.com/modelcontextprotocol/csharp-sdk)
- [Microsoft Agent Framework](https://github.com/microsoft/agent-framework)

## Contact

**Chris Ayers**, Principal Software Engineer
Azure EngOps AzRel

[Blog](https://chris-ayers.com/) | [GitHub](https://github.com/codebytes) | [LinkedIn](https://linkedin.com/in/chris-l-ayers/) | [Bluesky](https://bsky.app/profile/chris-ayers.com) | [Mastodon](https://hachyderm.io/@Chrisayers)

Licensed under the [MIT License](LICENSE).
